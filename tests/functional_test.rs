...
const TEST_BINARY_PATH_65C02_EXTENDED_OPCODES: &str =
    "tests/assets/65C02_extended_opcodes_test.bin";
...
const SUCCESS_ADDR_65C02_EXTENDED_OPCODES: u16 = 0x24F1; // Address of the "jmp *" success trap in 65C02_extended_opcodes_test
...
#[test]
fn klaus2m5_65c02_extended_opcodes_test() {
    ...
    while cpu.registers.program_counter != SUCCESS_ADDR_65C02_EXTENDED_OPCODES {
        let current_pc = cpu.registers.program_counter;
        ...
        assert!(
            ...
            current_pc,
            instr_count,
            SUCCESS_ADDR_65C02_EXTENDED_OPCODES,
            cpu.registers
        );
    ...
    // Success! The test reached the success trap (JMP to self at SUCCESS_ADDR_65C02)
    eprintln!(
        "Klaus2m5 65C02 functional test PASSED after {} instructions",
        instr_count
    );
}
